...
//创建一个GUI类
public class PandavfxGUI : ShaderGUI
{
    //自定义一个小按钮
...
//
        ...
        if (mainTex_Sampler.textureValue != null)
        {
            ...
            if (_Mainxx)
            {
                ...
                if (_tips == true)
                {
                    ...
                    style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                    GUILayout.Label(
                        "*贴图校色，可以拉大贴图明暗对比，制作更富色彩变化性的纹理，原理就是给原图一个power值，使其边缘更加锐利，在和原图lerp方式叠加起来，四个选项分别对应原图强度，修改图强度，修改图power值，原图修改图lerp混合程度（0-1之间即可）",
                        style);
                    EditorGUILayout.EndVertical();
                }
...

    {
...

        {
            ...
            if (_tips == true)
            {
                ...
                style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                GUILayout.Label(
                    "*包含normal默认UV，polar极坐标UV，cylinder圆筒UV三种模式，默认UV即模型自带UV，极坐标即中心向四周扩散UV，圆筒UV即世界坐标下一个圆筒包裹的UV", style);
                EditorGUILayout.EndVertical();
            }
        ...
        if (Dissolve_Tex.textureValue != null)
        {
            ...
            if (material.GetFloat("_IfDissolvePlus") == 1)
            {
                ...
                if (_tips == true)
                {
                    ...
                    style.fontSize = 10;
                    style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                    GUILayout.Label("*附加溶解图可以使用一张渐变图，比如从左到右的黑白渐变，这样溶解时就会有从左到右的溶解趋势，同理还可以使用其他方向或者一个扩散状的黑白渐变来增加溶解方向趋势感",
                        style);
                    EditorGUILayout.EndVertical();
                }
            ...
            if (_tips == true)
            {
                ...
                style.fontSize = 10;
                style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                GUILayout.Label("*开启自定义数据控制溶解偏移，开启自定义数据时请先添加custom1.xyzw,再添加custom2.xyzw，默认custom2.x和custom2.y控制溶解偏移程度",
                    style);
                EditorGUILayout.EndVertical();
            }
            ...
            if (_tips == true)
            {
                ...
                style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                GUILayout.Label(
                    "*包含normal默认UV，polar极坐标UV，cylinder圆筒UV三种模式，默认UV即模型自带UV，极坐标即中心向四周扩散UV，圆筒UV即世界坐标下一个圆筒包裹的UV，溶解图设置圆筒UV可以制作世界坐标系下的出现和消失效果",
                    style);
                EditorGUILayout.EndVertical();
            }
            ...
            if (_DistortTexxx)
            {
                ...
                if (material.GetFloat("_IfFlowmap") == 0)
                {
                    ...
                    if (_tips == true)
                    {
                        ...
                        style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                        GUILayout.Label(
                            "*开启后，将对扭曲贴图进行法线化处理，再对其他需要扭曲的纹理层进行扭曲，开启后扭曲的趋势将会是向四周扩散移动，不开启扭曲的趋势是向右上角移动，适合用来做屏幕扭曲的效果",
                            style);
                        EditorGUILayout.EndVertical();
                    }
                }
...

    {
        ...
        if (_tips == true)
        {
            ...
            style.fontSize = 10;
            style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
            GUILayout.Label("*用来制作顶点动画的遮罩，防止破面，比如球形的上下两个位置非常容易破面，这里我们只需要添加一个上下有一个渐变小黑条的遮罩图，就能让圆球上下两个位置不进行顶点偏移，从而防止破面",
                style);
            EditorGUILayout.EndVertical();
        }
...

    {
        ...
        if (_tips == true)
        {
            ...
            style.fontSize = 10;
            style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
            GUILayout.Label("*开启视差映射后，材质表面会有下凹的表现，非常适合用来制作地裂效果，特别注意如果使用的是粒子系统，不要使用公告板模式，要使用mesh，加一个plane片即可，否则旋转时会有bug",
                style);
            EditorGUILayout.EndVertical();
        }
...

    {
        ...
        GUILayout.Label(
            "本shader由油腻联盟坏熊猫制作，特别感谢闻亚洲，Nor_Zed，sion，123木头人，lolming，Allen，苏坤，AmantJy，仲冬，馒头,自赎,Cokey,J迷,AimerLily的帮助与支持",
            style);
        EditorGUILayout.EndVertical();
    }
}